...
> **Note (v1.1 update):** Automated tests were run using Python 3.11 + pytest. In Pydantic v2 (bundled with current FastAPI), `Field(gt=0)` rejects invalid amounts at the model validation layer and returns HTTP 422 — before the manual `HTTPException(400)` in the handler is reached. Tests TC-11, TC-12, and TC-18 were updated to expect HTTP 422 instead of HTTP 400. The API behaviour is correct and consistent — 422 is the more precise response for a schema constraint violation.

| Section | Tests | Passed | Failed | Pass Rate |
|---------|-------|--------|--------|-----------|
| A — Happy Path | 11 | 11 | 0 | 100% |
...
|-------|-------------|-------------|--------|
| AC-00 | GET /health returns 200 and running status | TC-00 | ✅ PASS |
| AC-01 | POST /income valid → 200 + success message | TC-01 | ✅ PASS |
| AC-02 | POST /income amount=0 → 422 (Pydantic v2) | TC-11 | ✅ PASS |
| AC-03 | POST /income amount=-50 → 422 (Pydantic v2) | TC-12 | ✅ PASS |
| AC-04 | POST /income missing source → 422 | TC-15 | ✅ PASS |
| AC-05 | POST /expense valid → 200 + success message | TC-02 | ✅ PASS |
| AC-06 | POST /expense amount=-1 → 422 (Pydantic v2) | TC-18 | ✅ PASS |
...
| AC-13 | Swagger UI loads at /docs with all 6 endpoints | TC-09 | ✅ PASS |
...
### TC-00 — GET /health confirms API is running
...
**Expected:** Page loads and displays 6 endpoints:
- `GET /health`
- `POST /income`
- `POST /expense`
- `GET /summary`
...
**Actual:** All 6 endpoints displayed. Each shows correct request schema, response model, and example values.
...
**Expected:** HTTP `422` — Pydantic v2 `Field(gt=0)` rejects amount=0 before handler is called

**Actual:** HTTP `422`

**Notes:** In Pydantic v2 (bundled with current FastAPI), the `gt=0` field constraint catches this at the model validation layer. The manual `HTTPException(400)` check in the handler is never reached. Both 400 and 422 are correct error responses — 422 is more precise as it identifies a schema constraint violation.
...
**Expected:** HTTP `422` — Pydantic v2 `Field(gt=0)` rejects negative amount before handler is called
...
**Notes:** The invalid request returned HTTP 422 (Pydantic v2 rejects negative amount at model layer) and was rejected before reaching the storage list. Confirmed that failed validation never modifies state.
...
The following pytest file is saved as `tests/test_main.py` and runs with `py -3.11 -m pytest tests/test_main.py -v`. It uses FastAPI's `TestClient` (via `httpx`) which runs the application in-process — no server required.

> **Environment note:** Python 3.15 (latest) does not yet have pre-built packages for pydantic-core. Use Python 3.11 to run the tests: `py -3.11 -m pytest tests/test_main.py -v`
...
**Actual output (verified March 2026):**
...
tests/test_main.py::test_expense_empty_category          PASSED
...
tests/test_main.py::test_invalid_does_not_affect_state   PASSED
